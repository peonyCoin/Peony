...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("peonycoin"))
        return false;
...
{
    // Convert peonycoin:// to peonycoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("peonycoin://"))
    {
        uri.replace(0, 12, "peonycoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PeonyCoin.lnk";
}
...
{
    return GetAutostartDir() / "PeonyCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("PeonyCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  PeonyCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("PeonyCoin-Qt"));
