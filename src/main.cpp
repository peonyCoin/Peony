...
#if defined(NDEBUG)
# error "PeonyCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "PeonyCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	...
	} else if (pindexBest->nHeight+1 > 1 && pindexBest->nHeight+1 <= 50) {
		nSubsidy = 0 * COIN;
	} else if (pindexBest->nHeight+1 > 50 && pindexBest->nHeight+1 <= 5000) {
		nSubsidy = 50 * COIN;
...
#ifdef ENABLE_WALLET
// PeonyCoin: attempt to generate suitable proof-of-stake
...
{
	RenameThread("PeonyCoin-loadblk");
